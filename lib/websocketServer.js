...
function attachWebsocketServerTo(server, settings) {
	...
	if (typeof wsServer.settings.protocol !== 'string') {
		wsServer.settings.protocol = 'nprotocol';
	}
	...
	wsServer.getAllChannels = function() {
		var allChannels = [];
		var theseChannels = [];
		for (var i=0; i<wsServer.connections.length; i+=1) {
			theseChannels = wsServer.connections[i].nserve.channels.get();
			allChannels = allChannels.concat(theseChannels);
		}
		return allChannels;
	};
	...
	wsServer.events.emit('websocketReady', wsServer);
	return wsServer;
};
...
function handleRequest(request) {  // wsServer scope
	...
	try {
		...
		var logData = { headers: request.httpRequest.headers, origin: request.origin, cookies: cookies, ip: request.remoteAddress, error: err };
		// log.njson(logData, 'wsconnectionerror_' + Date.now());
...
function createWsRes(message, connection, wsServer) {
	...
	wsRes.getHeader = function(key) {
		...
		// console.log('//////////////////////////////////// wsRes getHeader %s = %s', key, value);
		// console.log(connection.originalRequest.httpRequest.headers);
		return value;
	};
	wsRes.setHeader = function(key, value) {
		connection.originalRequest.httpRequest.headers[key] = value;
		// console.log('//////////////////////////////////// wsRes setHeader %s = %s', key, value);
		// console.log(connection.originalRequest.httpRequest.headers);

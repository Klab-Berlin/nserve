...
function attachWebsocketServerTo(server, settings) {
	...
	wsServer.log = settings.logger;
	wsServer.router = new WebSocketRouter();
	wsServer.router.attachServer(wsServer);
	wsServer.router.mount(
		'*',
		'vw-protocol', 
...
function originIsAllowed(request) {
	...
	if (typeof request.resourceURL.query.connectionId !== 'string') {
		return false;
	}
	// log.add(
		// 'allowing connection from origin {' 
			// + request.origin + '}', 
		// 'green', 
		// 'wsServer.listen',
		// 1
...
function handleRequest(request) {  // wsServer scope
	...
	if (
		...
		|| typeof request.resourceURL.query.clientId !== 'string'
	) {
	...
	connection.connectionId = minions.randomString(16, false, false, true);
	connection.request = request.webSocketRequest;
	connection.request.protocol = 'vw-protocol';
	...
	// vws.connections[connection.connectionId] = connection;
	this.log(

...
nserve.listen = function(settings) {
	...
	if (config.websocket === true) {
		srv._wsServer = websocketServer(srv, { protocol: config.protocol, logger: function() { nserve.events.emit('log', arguments); }, emitter: nserve.events });
	}
	...
	nserve.events.on(
		...
		function(server, req, res) {
			try {
				...
				return res.json(
					{
						errors: [ new Error('not able to parse your request'), err ],
						results: null,
						input: input
					}
				);

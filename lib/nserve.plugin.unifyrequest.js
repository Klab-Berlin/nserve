var ids = require('nserver.plugin.ids');

function unifyRequestData(req, connection) {
	var data = {
...
function getClientId(req, connection) {
	...
	if (typeof connection === 'object' && connection !== null) {
		connection.clientId = ids.ClientId.ensure(connection.clientId);
		data = connection.clientId;
	} else {
		req.clientId = ids.ClientId.ensure(req.clientId);
		data = req.clientId;
	}
	return data;
};
